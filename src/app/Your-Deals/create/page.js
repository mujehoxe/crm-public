...
function SuspenceWrapped() {
  ...
  useEffect(() => {
    ...
    if (toastShown == false) {
      toast.success(
        "Congratulations!! On closing your amazing new deal! Kindly fill out the form below to submit the transaction details.",
        {
          autoClose: 2000,
          closeButton: false,
        }
    ...
    if (buyerOneData.commisionttype === "%") {
      if (buyerOneData.Price && buyerOneData.Comission) {
        ...
        const commission = parseFloat(
          (buyerOneData.Comission ?? "0").replace(/,/g, "")
        );
      ...
      setBuyerOneData((prevState) => ({
        ...prevState,
        grandTotalCommission: parseFloat(
          (buyerOneData.Comission ?? "0").replace(/,/g, "")
        ),
      }));
    }
  ...
  useEffect(() => {
    ...
    buyerOneData.Comission,
    buyerOneData.grandTotalCommission,
    buyerOneData.VAT,
    buyerOneData.ComissionVAT,
    buyerOneData.netcom,
    buyerOneData.TotalComission,
    buyerOneData.commisionttype,
  ]);
  ...
  useEffect(() => {
    if (
      typeof buyerOneData.VAT !== "number" ||
      typeof buyerOneData.grandTotalCommission !== "number"
    ) {
      return;
    }
    ...
    let tnc = 0;
    const tic =
      Number(buyerOneData.VAT) + Number(buyerOneData.grandTotalCommission);

    let loyaltyBonusNumber = 0;
    if (typeof buyerOneData.loyaltyBonus === "string") {
      const loyaltyBonusWithoutCommas = buyerOneData.loyaltyBonus.replace(
        /,/g,
        ""
      );
  ...
  const handleSelectChange = (index, selectedOption) => {
    setTempAddedBuyerData((curr) => {
      if (Array.isArray(curr)) {
        return curr.map((buyer, id) =>
          id === index ? { ...buyer, Resident: selectedOption.value } : buyer
        );
      }
      return [];
    });
    ...
    if (emptyFields.length > 0 || imageErrors.length > 0) {
      ...
      const errorMessages = [
        ...(emptyFieldNames.length > 0
          ? [
              `Please fill all the mandatory fields for the last added buyer: ${emptyFieldNames.join(
                ", "
              )}`,
            ]
          : []),
  ...
  const handleInputChange = (index, field, value) => {
    setTempAddedBuyerData((curr) => {
      if (Array.isArray(curr)) {
        return curr.map((buyer, id) =>
          id === index ? { ...buyer, [field]: value } : buyer
        );
      }
      return [];
    });
            ...
            <div className="w-full flex flex-col justify-center items-center">
              ...
              {formCounter === 2 ? (
                radioBtnStatus === "offPlan" ? (
                  <div className="w-full max-w-[85%] mt-3">
                    <Accordion allowZeroExpanded preExpanded={["a"]}>
                      <AccordionItem uuid="a" className="mt-2">
                        <AccordionItemHeading className="bg-white py-2">
                          <AccordionItemButton className="flex font-bold w-full justify-between">
                            Buyer 1
                            <ImPlus />
                          </AccordionItemButton>
                        </AccordionItemHeading>
                        <AccordionItemPanel>
                          <div className="grid grid-cols-2 gap-x-5 gap-y-3 mt-4">
                            <div className="">
                              ...
                              <input
                                disabled
                                className="ml-4 rounded-md text-md focus:outline-none transition-all duration-200 focus:shadow-md bg-white px-3 py-0"
                                type="text"
                            ...
                            <div className="">
                              ...
                              <input
                                disabled
                                className="ml-4 rounded-md !border border-slate-300 text-md focus:outline-none transition-all duration-200 focus:shadow-md bg-white px-3 py-0"
                            ...
                            <div className="">
                              <label className="font-medium">Emirates ID</label>
                              <input
                                disabled={
                          ...
                          <div className="w-full mt-4">
                              ...
                              buyerOneData.Resident == "No" ? null : (
                                <div>
                                  <div className="flex items-center gap-4">
                                    <p className="block text-lg mb-0 leading-normal font-medium text-gray-900">
                                      Emirates ID
                                    </p>
                                    <div>
                                      <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-4 w-[450px] justify-between mt-2">
                              <p className=" text-lg !mb-0 leading-normal inline-block font-medium text-gray-900">
                                Visa{" "}
                    ...
                    <Accordion allowZeroExpanded>
                      {Array.isArray(tempAddedBuyerData) &&
                        tempAddedBuyerData.map((buyer, index) => (
                  ...
                  <div>you are at secondaryPlan</div>
                )
              ) : null}
                  ...
                  <div className="grid grid-cols-2 gap-x-5 gap-y-3 mt-4">
                    <div className="">
                      <label className="!mb-0">
                        EOI/Token Date<span className="text-red-500">*</span>
                      </label>
                      <input
                        className="ml-4 rounded-md !border border-slate-300 text-md focus:outline-none transition-all duration-200 focus:shadow-md bg-white px-3 py-0"
                        type="date"
                    ...
                    <div className="">
                      <label className="!mb-0">
                        Date of Closure <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="date"
                        onKeyDown={handleKeyDown}
                    ...
                    <div className="">
                      <label className="!mb-0">
                        No. of Bed <span className="text-red-500">*</span>
                      </label>
                      <input
                        className="ml-4 rounded-md !border border-slate-300 text-md focus:outline-none transition-all duration-200 focus:shadow-md bg-white px-3 py-0"
                        type="number"
                    ...
                    <div className="">
                      <label className="!mb-0">
                        Size /BUA Sq/Ft <span className="text-red-500">*</span>
                      </label>

                      <NumericFormat
                        value={buyerOneData.BUA}
                        onChange={(e) =>
                          setBuyerOneData({
                            ...buyerOneData,
                            BUA: e.target.value,
                    ...
                    <div className="">
                      ...
                      <input
                        className="ml-4 rounded-md !border border-slate-300 text-md focus:outline-none transition-all duration-200 focus:shadow-md bg-white px-3 py-0"
                        disabled={
                          buyerOneData.propertyType === "Apartment" ||
                          !buyerOneData.propertyType
                        }
                        type="text"
                        onChange={(e) =>
                          setBuyerOneData({
                    ...
                    <div>
                      <label className="!mb-0">
                        Ready / Offplan <span className="text-red-500">*</span>
                      </label>
                      <SearchableSelect
                        options={readyStatus}
                        onChange={(value) =>
                    ...
                    <div>
                      ...
                      <NumericFormat
                        value={parseFloat(
                          String(buyerOneData.Price).replace(/,/g, "")
                        )
                          .toFixed(2)
                          .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                        onChange={(e) =>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-x-2">
                        <NumericFormat
                          className="ml-4 rounded-md !border border-slate-300 text-md focus:outline-none transition-all duration-200 focus:shadow-md bg-white px-3 py-0 col-span-3"
                          value={
                            buyerOneData.commisionttype == "%"
                              ? buyerOneData.Comission
                              : parseFloat(
                                  String(buyerOneData.Comission).replace(
                                    /,/g,
                                    ""
                                  )
                                )
                    ...
                    <div>
                      ...
                      <NumericFormat
                        value={parseFloat(
                          String(buyerOneData.spotCash).replace(/,/g, "")
                        )
                          .toFixed(2)
                          .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                        onChange={(e) =>
                    ...
                    <div>
                      ...
                      <NumericFormat
                        disabled
                        value={parseFloat(
                          String(buyerOneData.grandTotalCommission).replace(
                            /,/g,
                            ""
                          )
                        )
                    ...
                    <div>
                      ...
                      <NumericFormat
                        disabled
                        value={parseFloat(
                          String(buyerOneData.VAT).replace(/,/g, "")
                        )
                          .toFixed(2)
                          .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                        className="ml-4 rounded-md !border border-slate-300 text-md focus:outline-none transition-all duration-200 focus:shadow-md bg-white px-3 py-0"
                    ...
                    <div>
                      ...
                      <NumericFormat
                        disabled
                        value={parseFloat(
                          String(buyerOneData.TotalComission).replace(/,/g, "")
                        )
                          .toFixed(2)
                          .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                        className="ml-4 rounded-md !border border-slate-300 text-md focus:outline-none transition-all duration-200 focus:shadow-md bg-white px-3 py-0"
                    ...
                    <div>
                      ...
                      <NumericFormat
                        value={parseFloat(
                          String(buyerOneData.loyaltyBonus).replace(/,/g, "")
                        )
                          .toFixed(2)
                          .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                        onChange={(e) =>
                    ...
                    <div>
                      ...
                      <NumericFormat
                        disabled
                        value={parseFloat(
                          String(buyerOneData.netcom).replace(/,/g, "")
                        )
                          .toFixed(2)
                          .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                        className="ml-4 rounded-md !border border-slate-300 text-md focus:outline-none transition-all duration-200 focus:shadow-md bg-white px-3 py-0"
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <div className="input-group">
                                ...
                                <input
                                  onChange={(e) =>
                                    handleFileChange2("0", 0, e.target.files)
                                  }
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <div className="input-group">
                                ...
                                <input
                                  onChange={(e) =>
                                    handleFileChange2("1", 0, e.target.files)
                                  }
                                  className="text-sm text-gray-900 border border-gray-300 cursor-pointer focus:outline-none"
                                  aria-describedby="file_input_help"
                                  id="booking"
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <div className="input-group">
                                ...
                                <input
                                  onChange={(e) =>
                                    handleFileChange2("2", 0, e.target.files)
                                  }
                                  className="text-sm text-gray-900 border border-gray-300 cursor-pointer focus:outline-none"
                                  aria-describedby="file_input_help"
                                  id="spa"
                    ...
                    >
                      {submitting ? (
                        <div role="status">
                          <svg
                            aria-hidden="true"
                            className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-miles-600"
                            viewBox="0 0 100 101"
                            fill="black"
              ...
              {formCounter === 4 && (
                <div className="w-full mt-5 h-[65svh] flex  flex-col items-center justify-center">
                  <p className="text-3xl text-red-600 w-full text-center">
                    Congratulations !! Return to dashboard <br />
                    You have successfully submitted your deal
                  </p>
