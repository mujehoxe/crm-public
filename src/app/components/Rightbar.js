...
const Rightbar = ({ userData, setSettingsBarOpen }) => {
  ...
  return (
    <div className="flex grow h-screen flex-col gap-y-5 overflow-y-auto border-l border-gray-200 bg-white px-6">
      ...
      <nav className="flex flex-1 flex-col">
        <ul role="list" className="flex flex-col relative space-y-2 p-2 h-full">
                ...
                <div className="flex gap-x-4 justify-between">
                  <item.icon
                    className={classNames(
                      item.current
                        ? "text-miles-600"
                        : "text-gray-600 group-hover:text-miles-700",
                      "h-6 w-6 shrink-0"
                    )}
                    aria-hidden="true"
                  />
