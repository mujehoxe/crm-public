			...
			<div className="inline-block min-w-full align-middle">
				<div className="w-full h-[36rem] flex">
							...
							{agents.map((agent) => (
								<li key={agent.email}
										...
										className={`flex rounded-md justify-between gap-x-4 px-2 py-4 ${
											agent._id === selectedAgent ? "bg-miles-100 hover:bg-miles-200" : "hover:bg-gray-100"}
										`}>
									<div className="flex space-x-2 items-center">
										{agent?.Avatar ?
											<img
												...
												alt=""/> :
											<FaRegUserCircle className="h-12 w-12 text-gray-300"
																			 aria-hidden="true"/>
										}
										<div className="min-w-0">
											<p
												className="text-sm font-semibold leading-6 text-gray-900">{agent.username}</p>
											<p
												className="mt-1 truncate text-xs leading-5 text-gray-500">{agent.email}</p>
										</div>
									</div>
									<div
										className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
										<p
											className="text-sm leading-6 text-gray-900 mb-1">{agent.Role}</p>
										{Object.values(onlineAgents).filter(a => a.id === agent._id).length > 0 && (
											<div className="flex items-center gap-x-1.5">
												<div
													className="flex-none h-min rounded-full bg-emerald-500/20 p-1">
													<div
														className="h-1.5 w-1.5 rounded-full bg-emerald-500"/>
												</div>
												<p
													className="text-xs leading-5 text-gray-500">Online</p>
											</div>
										)}
									</div>
								</li>
					...
					<div className="flex-grow">
						...
						<MapContainer center={[25.276987, 55.296249]} zoom={10}
													className="h-full" ref={mapRef}>
							<TileLayer
								...
								<CustomMarker
									...
									popupContent={
										<div>
											...
											<p>Last Updated: {new Date(agent.timestamp).toLocaleString()}</p>
										</div>
									}
								>
							...
							{selectedAgent && agentHistory[selectedAgent] && (
								<Polyline
									positions={agentHistory[selectedAgent].map(point => [point.lat, point.lng])}
									color="red"
									weight={1}
								/>
							)}
