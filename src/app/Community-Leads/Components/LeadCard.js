...
export default function LeadCard({
	...
	const [isUpdateDescriptionInput, setIsUpdateDescriptionInput] = useState(false);
	...
	async function handleUpdateSubmit() {
		setLoading(true);
		try {
			const response = await axios.patch("/api/Lead/update/" + lead._id, updateBody);
			response.status === 200 && (setIsUpdateDescriptionInput(false) || setUpdateBody({
				...updateBody,
				updateDescription: ""
			}) || setCurrentPageLeads((prev) => prev.map((prevLead) => {
	...
	return <li
		className={`relative col-span-1 divide-y divide-gray-200 h-min rounded-lg bg-white shadow hover:shadow-lg ${selectedLeads.includes(lead) ? "border-blue-400" : "hover:border-gray-400"}`}
		ref={divRef}
			...
			<div className="space-y-1">
				...
				<div className="flex justify-between space-x-1 items-start">
					<span
						...
						className={`text-sm text-gray-900 w-full rounded-md min-h-[40px] line-clamp-2 ${isDescriptionInput && "border-2 border-gray-300 px-1"}`}
					>
